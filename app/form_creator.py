    ...
    if form.is_submitted():
        ...
        for key in sweep_keys:
            if request.form[key]:
                ...
                filled_fields = sum(
                    1
                    for suffix in ("_begin", "_step", "_end")
                    if request.form[base_key + suffix]
                )

                # If not all three fields (begin, step, and end) are filled, show a flash message and render the form again
                if filled_fields < 3:
                    flash(
                        f"{base_key.replace('_', ' ').title()} sweep form isn't filled out completely.",
                        "success",
                    )
                    return render_template(
                        "form.html", title=formatted_title, form=form, length=length
                    )
                ...
                if key.endswith("_begin"):
                    sweep_toggled = True
                    values[base_key] = (
                        Decimal(request.form[base_key + "_begin"]),
        ...
        if sweep_toggled:
                ...
                for i in range(len(filtered_keys)):
                    if filtered_keys[i] in values:
                        ...
                        if request.form[filtered_keys[i]] != "":
                            car[filtered_keys[i]] = Decimal(
                                request.form[filtered_keys[i]]
                            )
            ...
            resp = make_response(
                redirect(
                    url_for("output_bp.output", module=module, operation=operation)
                )
            )
...
def output():
    ...
    if sweep_toggled:
        ...
        sweep_combos = tuple(
            tuple(x) for x in json.loads(session.get("sweep_combos_str"))
        )
        time_data = []
        brake_info = []
        caliperBrands = []
        ...
        if operation.__name__ == "brake_input":
            response = make_response(
                render_template(
                    ...
                    values=values,
                    caliperBrands=caliperBrands,
                    padBrand=padBrand,
                    caliperModel=caliperModel,
                    padModel=padModel,
                )
            )
        else:
            if len(value_names) == 2:
                isocontour_data, layout = generate_graph(
                    time_data, sweep_combos, value_names
                )
            ...
            response = make_response(
                render_template(
                    ...
                    sweep_combos=sweep_combos,
                    values=values,
                    isocontour_data=isocontour_data_json,
                    layout=layout_json,
                )
            )
        ...
        car = Car.from_file(data)
        if operation.__name__ == "brake_input":
            brake_info, caliperBrands_np, padBrand_np, caliperModel_np, padModel_np = (
                operation(car)
            )
            ...
            response = make_response(
                render_template(
                    "output.html", time_data=time_data, sweep_combos=[], values={}
                )
            )
