...
car = Car.from_file("data/rm28.json")
...
class Competition:
    def __init__(self, autox_file, endurance_file):
        ...
        self.Acceleration = Track(
            np.linspace(0, 75, int(75 / resolution) + 1),
            np.zeros(int(75 / resolution + 1)),
        )
        self.Skidpad = Track(
            np.linspace(0, 180.4, int(180.4 / resolution) + 1),
    ...
    def plot_battery(self, solution):
        ...
        ax1.tick_params(axis="y", labelcolor="black")
        ax1.plot(
            solution[:, 0], solution[:, 4] / 1000, label="P (kW)", color="gold", ls="-"
        )
        ax1.plot(
            solution[:, 0],
            solution[:, 8] / 1000,
        ...
        ax1.tick_params(axis="y", labelcolor="black")
        ax1.plot(
            solution[:, 0], solution[:, 10], label="T (C)", color="tab:cyan", ls="-"
        )
        ax1.set_xlim(0, solution[np.shape(solution)[0] - 1, 0])
        ax1.set_ylim(35, 70)
    ...
    def plot_power_limits(self, Pmin, Pmax, count=100):
        ...
        for i in range(Pmin, Pmax, int((Pmax - Pmin) / count)):
            ...
            n += 1
        ax1.plot(
            result[:, 0] / 1000, result[:, 1], label="T (C)", color="tab:cyan", ls="-"
        )
        ax2.plot(
            result[:, 0] / 1000,
            result[:, 2],
    ...
    def sweep_var(self, car, xvar, yvar, min, max, count=50):
        ...
        for i in range(x.size):
            ...
            df = pd.DataFrame(
                np.array([times, dQ, sol[:, 10]]).T,
                columns=["t(s)", "dQ/dt(W)", "T(C)"],
            )
            df.to_csv("data/heat_gen/" + str(int(x[i])) + "W_limit.csv")
        return x, y


MIS_2019 = Competition(
    "data/2018MichiganAXTrack_new.csv", "data/2019MichiganEnduranceTrack.csv"
)
